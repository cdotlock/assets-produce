#!/usr/bin/env python3
"""Hermetic contract test for tools/hole-fill/hole-fill.py --mock entry.

This suite MUST run on plain python3 with NO cv2/scipy/numpy installed for the
...
Run:  python3 -m pytest tools/hole-fill/test_hole_fill_mock.py -q
"""
...
TOOL = Path(__file__).resolve().parent / "hole-fill.py"
...
def test_mock_file_input(smoke_dir: Path) -> None:
    ...
    assert doc["meta"]["atomic_tool"] == "hole-fill"
...
def test_mock_meta_echoes_params(smoke_dir: Path) -> None:
    """Meta block must echo back the dilate/min_size/max_size params used."""
    ...
    doc = json.loads(proc.stdout)
    assert doc["meta"]["dilate"] == 3
    assert doc["meta"]["min_size"] == 150
    assert doc["meta"]["max_size"] == 5000
    _assert_valid_1x1_rgba_png(Path(doc["output"]["path"]))
...
def test_explicit_cli_input_output(smoke_dir: Path) -> None:
    ...
    proc = _run(
        ["--input", str(src), "--output", str(dst), "--dilate", "2", "--min-size", "200", "--max-size", "8000"]
    )
...
# ---------------------------------------------------------------------------
# Negative: non-integer numeric params (HAZARD 2)
# ---------------------------------------------------------------------------

def test_negative_non_integer_dilate(smoke_dir: Path) -> None:
    """A non-int-coercible dilate -> INVALID_INPUT exit 2."""
    payload = {
        "input_path": str(smoke_dir / "in.png"),
...
def test_negative_non_integer_min_size(smoke_dir: Path) -> None:
    """A non-int-coercible min_size -> INVALID_INPUT exit 2."""
    payload = {
        "input_path": str(smoke_dir / "in.png"),
        "output_path": str(smoke_dir / "out.png"),
        "min_size": "bad",
...
def test_negative_non_integer_max_size(smoke_dir: Path) -> None:
    """A non-int-coercible max_size -> INVALID_INPUT exit 2."""
    payload = {
        "input_path": str(smoke_dir / "in.png"),
        "output_path": str(smoke_dir / "out.png"),
        "max_size": None,
