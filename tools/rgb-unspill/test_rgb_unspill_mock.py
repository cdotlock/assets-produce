#!/usr/bin/env python3
"""Hermetic contract test for tools/rgb-unspill/rgb-unspill.py --mock entry.

This suite MUST run on plain python3 with NO numpy / Pillow installed for the
--mock PNG path (mock writes a stdlib-only 1x1 RGBA PNG).  For the --mock WebP
path, Pillow IS required (no stdlib WebP encoder -- precedent from
tools/hybrid-to-webp).  Pillow is also used in the round-trip assertions.

Run:  python3 -m pytest tools/rgb-unspill/test_rgb_unspill_mock.py -q
"""
...
TOOL = Path(__file__).resolve().parent / "rgb-unspill.py"
...
def _assert_valid_1x1_rgba_webp(path: Path) -> None:
    """Assert the produced WebP is a valid 1x1 RGBA image using Pillow.

    Pillow is required here -- stdlib cannot decode WebP.
    The mock WebP is also written via Pillow (precedent: tools/hybrid-to-webp).
    """
    assert path.is_file(), f"output WebP not created: {path}"
    from PIL import Image
    img = Image.open(path).convert("RGBA")
    assert img.size == (1, 1), f"expected size (1, 1), got {img.size}"
    assert img.mode == "RGBA", f"expected mode RGBA, got {img.mode}"
...
# ---------------------------------------------------------------------------
# Mock file-entry (PNG output)
# ---------------------------------------------------------------------------

def test_mock_file_input_png(smoke_dir: Path) -> None:
    ...
    assert doc["meta"]["atomic_tool"] == "rgb-unspill"
...
# ---------------------------------------------------------------------------
# Mock stdin-entry (PNG output)
# ---------------------------------------------------------------------------

def test_mock_stdin_input_png(smoke_dir: Path) -> None:
...
# ---------------------------------------------------------------------------
# Round-trip: WebP output mock
# ---------------------------------------------------------------------------

def test_mock_webp_output(smoke_dir: Path) -> None:
    """Mock with .webp output_path -- Pillow writes a tiny valid WEBP placeholder."""
    out = smoke_dir / "out.webp"
    payload = {
        "input_path": str(smoke_dir / "in.webp"),
        "output_path": str(out),
        "overwrite": True,
        "mock": True,
    }
    ...
    assert doc["meta"]["atomic_tool"] == "rgb-unspill"
    assert doc["meta"]["mock"] is True
    assert doc["meta"]["format"] == "webp"

    _assert_valid_1x1_rgba_webp(Path(doc["output"]["path"]))


# ---------------------------------------------------------------------------
# Meta echoes format field
# ---------------------------------------------------------------------------

def test_mock_meta_echoes_format_png(smoke_dir: Path) -> None:
    """Meta block must include format=png for .png output."""
    out = smoke_dir / "out_fmt.png"
    payload = {
        "input_path": str(smoke_dir / "in.png"),
        "output_path": str(out),
        "overwrite": True,
...
# ---------------------------------------------------------------------------
# Negative: unsupported output extension -> INVALID_INPUT exit 2
# ---------------------------------------------------------------------------

def test_negative_unsupported_output_extension(smoke_dir: Path) -> None:
    """An output_path with an unsupported extension -> INVALID_INPUT exit 2."""
    out = smoke_dir / "out.bmp"
    payload = {
        "input_path": str(smoke_dir / "in.png"),
        "output_path": str(out),
        "mock": True,
...
def test_explicit_cli_input_output(smoke_dir: Path) -> None:
    ...
    proc = _run(["--input", str(src), "--output", str(dst)])
