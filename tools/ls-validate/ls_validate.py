#!/usr/bin/env python3
"""
ls-validate — frozen-subprocess bridge for the canonical LS validator.

Wraps the sha256-pinned Go binary from lunascripts@b36a407 (vendored under
tools/ls-validate/lunascripts/).  Never reimplements validation logic;
always delegates to the upstream binary (design D6 / spec §8.2).

Input (JSON object on stdin via --input -):
  script_path : str  — absolute path to the .ls / .md file to validate
  content     : str  — raw LS text (written to a tempfile, mutually exclusive with script_path)
...
Mock sentinel:
  If content contains the literal string __LS_MOCK_FAIL__ OR script_path ends with
  __LS_MOCK_FAIL__, the mock branch returns a canned FAIL verdict.  Otherwise it
...
Drift guard:
  ...
  tools/ls-validate/lunascripts/** (sorted by path, LC_ALL=C bytewise) and
...
Build-once cache:
  ...
  $TMPDIR/assets-produce-ls-validate/<manifest_sha>/ls.  Subsequent calls reuse
...
VENDOR_DIR = TOOL_DIR / "lunascripts"
MANIFEST_PATH = TOOL_DIR / "FROZEN_MANIFEST.sha256"


@functools.lru_cache(maxsize=1)
def _recompute_manifest() -> str:
    """Recompute the sha256 manifest over lunascripts/** and return it as text.

    Algorithm must reproduce the committed FROZEN_MANIFEST.sha256 byte-for-byte:
      - enumerate every regular file under tools/ls-validate/lunascripts/
      - sha256 each file's bytes
      - format each line as: "<64-hex-sha256>  <relpath-from-tools/ls-validate/>"
        (two spaces; relpath is POSIX-style, starting with lunascripts/)
...
def _check_drift() -> str | None:
    ...
    if recomputed != committed:
        return (
            ...
            "Re-vendor from lunascripts@b36a407 and regenerate the manifest."
        )
    return None
...
def _build_cache_bin() -> Path:
    """Build the lsc binary (once) and return its path.
    ...
    cache_dir = Path(tempfile.gettempdir()) / "assets-produce-ls-validate" / manifest_sha
    cache_dir.mkdir(parents=True, exist_ok=True)
    bin_path = cache_dir / "ls"
    ...
    try:
        ...
        raise RuntimeError(
            "go binary not found. Install Go >= 1.23.4 to use ls-validate in non-mock mode."
        )

    result = subprocess.run(
        [go_exe, "build", "-o", str(bin_path), "./cmd/lscc"],
        cwd=str(VENDOR_DIR),
        capture_output=True,
        text=True,
        timeout=120,
...
def _validate_with_binary(target: Path) -> tuple[str, list[str], str]:
    """Run `lsc validate <target>` and translate to (verdict, errors, raw).
    ...
    try:
        ...
        raise RuntimeError(f"ls subprocess failed: {e}") from e
...
def _run_json_main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="ls-validate")
    ...
    if mock:
        is_fail = (
            (content is not None and "__LS_MOCK_FAIL__" in content)
            or (script_path is not None and script_path.endswith("__LS_MOCK_FAIL__"))
        )
        if is_fail:
            result = {
                "verdict": "FAIL",
    ...
    try:
        if content is not None:
            fd, tmp_name = tempfile.mkstemp(suffix=".md", prefix="ls_validate_")
            os.close(fd)
    ...
    if verdict == "PASS":
        result = {
            "verdict": "PASS",
            "errors": [],
            "meta": {"atomic_tool": "ls-validate", "mock": False},
        }
    else:
        result = {
            "verdict": "FAIL",
