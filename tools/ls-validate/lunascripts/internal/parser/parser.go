...
func (p *Parser) parseDirective() (ast.Node, error) {
	...
	case "on":
		return nil, fmt.Errorf("line %d col %d: @on is not a LS directive — use @if (check.success) / @if (rating.X) inside brave options and minigames", p.cur.Line, p.cur.Col)
	default:
...
// scene. Body is standard LS; branching on the result uses
