"""
Hermetic tests for ls_validate.py — MUST pass on plain python3 with NO external packages.

Run:  python3 -m pytest tools/ls-validate/test_ls_validate_mock.py -v
      (or:  cd tools/ls-validate && python3 -m pytest test_ls_validate_mock.py -v)
"""
...
BRIDGE = TOOL_DIR / "ls_validate.py"
MANIFEST = TOOL_DIR / "FROZEN_MANIFEST.sha256"


def _load_module():
    spec = importlib.util.spec_from_file_location("ls_validate", BRIDGE)
...
def test_mock_normal_returns_pass():
    ...
    assert out["errors"] == []
    assert out["meta"]["atomic_tool"] == "ls-validate"
    assert out["meta"]["mock"] is True


# ---------------------------------------------------------------------------
# Test 2 — --mock with __LS_MOCK_FAIL__ sentinel -> FAIL verdict, exit 0
# ---------------------------------------------------------------------------

def test_mock_sentinel_content_returns_fail():
    payload = json.dumps({"content": "__LS_MOCK_FAIL__", "mock": True})
    p = _run(payload)
...
def test_mock_sentinel_in_script_path_returns_fail():
    payload = json.dumps({"script_path": "/tmp/__LS_MOCK_FAIL__", "mock": True})
    p = _run(payload)
...
def test_mock_flag_cli_arg_with_sentinel():
    ...
    payload = json.dumps({"content": "some __LS_MOCK_FAIL__ text"})
...
def test_drift_guard_fires_on_tampered_expected_manifest(monkeypatch, tmp_path):
    ...
    lunascripts/** tree, so recompute != tampered-expected -> drift. The
    real lunascripts/** and FROZEN_MANIFEST.sha256 are NEVER modified.
    """
    mod = _load_module()
