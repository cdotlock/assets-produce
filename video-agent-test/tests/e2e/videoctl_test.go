...
func TestVideoctlSubmitDryRunWritesRequest(t *testing.T) {
	...
	cmd := exec.Command("go", "run", "./scripts/videoctl", "submit", promptPath, "--dry-run", "--run-dir", runDir, "--json")
	cmd.Dir = repo
...
func TestVideoctlSubmitWaitAgainstFakeGateway(t *testing.T) {
	...
	cmd := exec.Command("go", "run", "./scripts/videoctl", "submit", promptPath, "--wait", "--timeout", "5", "--poll", "1", "--run-dir", runDir, "--allow-non-oss", "--json")
	cmd.Dir = repo
