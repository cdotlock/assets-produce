...
The old Python helper scripts have been removed. `scripts/videoctl` is the only
source entrypoint, and the compiled local binary is `scripts/bin/videoctl`.

Agents should read `scripts/videoctl/AGENT_REFERENCE.md` before using the CLI.
...
`scripts/bin/` is ignored by git. Rebuild locally whenever the CLI changes.
...
scripts/bin/videoctl upload <file...> --folder public/image
scripts/bin/videoctl upload <file...> --no-sidecar
scripts/bin/videoctl upload <file...> --json
```
...
scripts/bin/videoctl payload <prompt.md> --allow-non-oss
scripts/bin/videoctl payload <prompt.md> --allow-text-only
```
...
scripts/bin/videoctl validate <prompt.md> --allow-non-oss
scripts/bin/videoctl validate <prompt.md> --allow-empty
scripts/bin/videoctl validate <prompt.md> --json
```
...
```bash
scripts/bin/videoctl submit <prompt.md> --dry-run
scripts/bin/videoctl submit <prompt.md> --dry-run --run-dir <dir>
scripts/bin/videoctl submit <prompt.md> --wait
scripts/bin/videoctl submit <prompt.md> --wait --timeout 1200 --poll 30
scripts/bin/videoctl submit <prompt.md> --force
scripts/bin/videoctl submit <prompt.md> --resume-latest
```
...
```bash
scripts/bin/videoctl status <run-dir>
scripts/bin/videoctl status <run-dir> --json
```
...
scripts/bin/videoctl download --url-file <video.url> --out <shot.mp4>
scripts/bin/videoctl download <video_url> --out <shot.mp4> --no-sidecar
```
...
scripts/bin/videoctl extract-end-frame <shot.mp4> <shot_end.png> --offset 0.1
scripts/bin/videoctl extract-candidates <shot.mp4> <out_dir> --shot-id shot_1
scripts/bin/videoctl extract-candidates <shot.mp4> <out_dir> --shot-id shot_1 --interval 2
scripts/bin/videoctl select-spatial-frame <candidate.png> <shot_spatial.png>
```
...
```bash
scripts/bin/videoctl run-shot <prompt.md> --download --extract-end-frame
scripts/bin/videoctl run-shot <prompt.md> --force --download --extract-end-frame
```
...
1. build the binary with `make build` if `scripts/bin/videoctl` is missing
2. upload local media with `scripts/bin/videoctl upload`
3. validate every approved prompt with `scripts/bin/videoctl validate`
4. submit with `scripts/bin/videoctl submit <prompt.md> --wait`
