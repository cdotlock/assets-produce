...
### 8.1 LLM
- **主脑**：Claude（开 prompt cache）⚠ § 15 / 1.16：主脑默认改经 `mob-ai` 网关（OpenAI 兼容，`https://ai.mob-ai.cn/v1`）；Anthropic ephemeral prompt cache 经此网关实测不生效，"开 prompt cache" 仅在官方直连路径成立
...
- prompt cache 在 Claude 调用时确实命中（log 验证）⚠ § 15 / 1.16：本项仅官方直连路径成立；主脑经 mob-ai 网关时 Anthropic ephemeral prompt cache 实测不可达（litellm OpenAI-compat 不透传，cache_creation/read 恒 0），不影响功能
...
- 5 份 skill body 草稿到 `knowledge/asset-generation/`（character-portrait / scene-bg / cg-render / cover / shot-image-from-ls）
...
- 不接 novels-to-lunascript / lunaverse-backend
- 不实现 callback / 自动重试
...
**目标**：把 `lunaverse-backend/generate-upscale-matting/` 中纯素材生产工具（CG 渲染 / 批量 upload / upscale）搬到 assets-produce 顶层 `tools/`，让 Phase 8 已落地的 mini agent loop 能力完整。详细设计见 [`2026-05-14-three-repo-asset-integration-design.md`](2026-05-14-three-repo-asset-integration-design.md) § 11 Phase 9。
...
- lunaverse-backend 对应文件加 DEPRECATED 注释（**不删**；删交给 backend 维护方）
...
**目标**：把 novels-to-lunascript 与 lunaverse-backend 实际接通 assets-produce 对外 API。详细设计见 [`2026-05-14-three-repo-asset-integration-design.md`](2026-05-14-three-repo-asset-integration-design.md) § 11 Phase 10。

**范围**：

- novels-to-lunascript：新增 `src/clients/assets_produce_client.py`（仅 lookup）；LS `asset_ref` 加可选 id/name/kind；可选 `ls-verify --resolve-assets` flag；`.env.example` 加 `ASSETS_PRODUCE_BASE_URL` / `ASSETS_PRODUCE_TOKEN`
- lunaverse-backend：新增 `app/upstream/assets-produce-http.ts`；修改 `app/upstream/agent-forge-client.ts` real branch（throw 改为 HTTP fetch）；不动 `assets-remix-service.ts` 接口；`.env.example` 加变量；README 写 `ASSETS_REMIX_MODE=real` 切换说明
...
- novels-to-lunascript `assets_produce_client.py` 单元覆盖 happy / 404 / 401 / 网络错
- lunaverse-backend `assets-produce-http.ts` 单元覆盖 4 个操作 + 错误矩阵
- 用户本机一条 e2e 走通：LS → backend remix → assets-produce real → 拿 url → 小程序看到图
...
- `generate-sfx-elevenlabs`（**完整真实实现**）：移植 novels-to-lunascript `skills/sfx-normalizer/elevenlabs_generator.py::ElevenLabsGenerator.generate()` 的合成调用（`POST /v1/sound-generation`，`xi-api-key`，同步 mp3 字节），**不**带其 clusterer/orchestrator/dry_run-占位 等耦合件
...
- 不迁 n2m 聚类/归类；不改 novels-to-lunascript 本身
...
**目标**：把 lunaverse-backend 图片处理套件迁回 assets-produce 作 Phase 9 式 Python 原子工具。详细设计见 [`2026-05-15-audio-and-asset-parity-design.md`](2026-05-15-audio-and-asset-parity-design.md) § 6。

**范围**：

- `lunaverse-backend/generate-upscale-matting/` 内 matting/cutout/hole_fill/spill/格式转 等 → `tools/<name>/`（JSON I/O + `--mock` + `python-runner.ts` 桥）
...
- 不动 novels-to-lunascript / lunaverse-backend
...
| 1.10 | 2026-05-14 | 用户要求把 assets-produce 与 `cdotlock/novels-to-lunascript` 和 `cdotlock/lunaverse-backend` 真正接通，并把 backend 内的素材生产能力（CG / sync-to-oss / upscale）迁回 assets-produce。约束：lunaverse-backend 不是用户维护，最小改动；novels-to-lunascript 与 assets-produce 由用户维护，可正常改造；技术要 AI-native（mini agent loop + skill 编排，不写新的硬编码 service）。新增 Phase 8 / 9 / 10：Phase 8 在 assets-produce 内立起 4 个对外操作（`asset.create` / `asset.status` / `asset.lookup` / `asset.catalog.since`）REST + MCP 双层；Phase 9 把 CG / OSS-sync / upscale 三件工具搬到 `tools/`，挂上 atomic tools；Phase 10 让 novels-to-lunascript 加 lookup client、lunaverse-backend `agent-forge-client.ts` real mode 切到 HTTP to assets-produce（不动 `assets-remix-service.ts` 接口、不动 outbox/BullMQ）。完整设计见 `2026-05-14-three-repo-asset-integration-design.md`。影响范围：assets-produce 对外形态、跨仓集成；不改 § 2 原子能力 + skill 编排原则（mini agent loop 本质就是 LLM + skill body + atomic tools 的运行回路，非硬编码流水线 service），不改 § 11.4 接口稳定（Phase 8 落地后 4 个对外操作即 lock-in 起点）。 | cdotlock + Claude |
...
| 1.15 | 2026-05-18 | 用户要求把"做 novels-to-lunascript 最终 demo（书 no-rules-in-bad-ideas）时实际用的那一整套 prompt"完整迁进 assets-produce 自维护，迁移（verbatim 冻结进 `knowledge/style-prompts/`，commit `d4035cc`/`21890a3`）已完成；本条记录后续 **B1** 决定——把这套 prompt 真正接进真生成路径。Brainstorming 核实并纠正前提：决定成图的是 lunaverse-backend `styles` 表 **YA_Impasto** 家族（非 style-prompts MCP 的 korean-manga，后者仅 06 阶段 appearance 脚手架）；NRBI 标准为三阶段（Phase 1 render-with-style.py 出图 / Phase 2 Real-ESRGAN upscale / Phase 3 "v10" MODNet+4 步抠图），canonical 流程文档 = `lunaverse-backend/.claude/skills/generate-asset/SKILL.md`（只读参考，不改）。assets-produce 现状：Phase 2 = `upscale-spec`（Phase 12）、Phase 3 = Phase 13 matting/cutout + `oss-put`（Phase 12）均已落地；**唯独 Phase 1 出图未接 = B1 范围**。"v10" 专指 Phase-3 后处理标准（`matting.py` `V10_*`，Phase 13 已港，`GREEN_SCREEN_RGB=(0,255,0)`），故绿幕 hex 非开放项——B1 复刻 `render-with-style.py` 改写后输出 `#00FF00`（NRBI 实际成图值、Phase-13 抠图入参值；上游 `#00B140` 是脚本已改写掉的陈旧值）。用户决策（brainstorming 锁定 D1–D6）：D1 **严格复刻**（逐字、确定性拼装，LLM 仅填可变 appearance/scene 文本）；D2 **全集**（Layer A 系列立绘 / A.5 换装锚 / B 场景宫格 / C 场景方图 / D 场景变体 / E EP sprite）；D3 绿幕 `#00FF00`；D4 拼装器 = **冻结 Python 子进程**（新原子工具 `nrbi-render-prompt` 直接 shell 冻结 `render-with-style.py` 的 prompt 装配代码，逐字一致是构造保证，同 `cg-render` 模式；砍 `load_styles` SSH/PG→读本地 `styles.json`、砍 keyring/Zenmux 图像调用、砍 legacy `main()` 批处理→单任务 JSON I/O；`extract_appearance`/`build_sprite_text`/`_ANCHOR_*`/`clean_anchor_prompt`/`rebuild_grid_prompt`/`_SCENE_SQUARE_TEMPLATE`/`clean_sprite_prompt`/`normalize_prompt_for_style`/模板填充 **verbatim 不动**）；D5 **不新增 AssetKind**（新 skill body 注册进 `ASSET_GENERATION_SKILLS`、skill_hint 驱动，沿用 Phase 12/13 no-kind 先例）；D6 风格参考图用 `styles.json.reference_urls` 远程阿里云 OSS **原样**（本地化作后续加固）。落地物：`nrbi-render-prompt` 原子工具 + 4 个 skill body（`character-portrait-spec`/`scene-bg-spec` 改、`outfit-anchor-spec`/`ep-sprite-spec` 新）+ `intent-to-skill.ts` 注册 2 个新 skill + golden 逐字断言测试（73 条 `nrbi-anchor_tasks.json` + 非 anchor 层冻结脚本 dryRun fixture）。详细设计见 `2026-05-18-b1-nrbi-render-prompt-design.md`。影响范围：注入缝后新增 1 原子工具 + 4 skill body + 注册；**不改** § 2 原子能力 + skill 编排原则（`nrbi-render-prompt` 为 LLM 经 skill 编排调用的确定性原子能力，非硬编码流水线 service；整本书自动 DAG 编排器明确不做）、**不改** § 11.4 任何对外接口（无新 AssetKind / 无 REST / DB / error code 变更）、不动 Phase 2/3、不动 Phase 14 LLM 循环（加 body + 注册即可，零 loop 代码改动）、不动 lunaverse-backend。 | cdotlock + Claude |

| 1.16 | 2026-05-19 | 用户要求把 Claude 主脑从直连 Anthropic 官方改为走公司 `mob-ai` 网关路径。Ground-truth 核实(本 session 实测,非推断):(a) 本机 `~/.zshrc` 的 `cc()` 函数证实公司网关 = `https://ai.mob-ai.cn`;`novels-to-lunascript/docs/superpowers/specs/2026-05-09-llm-gateway-migration.md` + `lunaverse-backend/.env`(`# LLM provider（OpenAI 兼容 gateway）`)证实该网关已被全平台采用、有 smoke 实证。(b) 实测三种接法:`POST https://ai.mob-ai.cn/v1/chat/completions` + `Authorization: Bearer` + `model=claude-sonnet-4-6` → **HTTP 200 正常返回**;`POST /v1/messages` Anthropic 原生协议(`x-api-key` 与 `Bearer` 两种头都试)→ **HTTP 400 `litellm.BadRequestError`**。结论:mob-ai 后端是 litellm,**仅暴露 OpenAI 兼容协议,不支持 Anthropic 原生 `/v1/messages`**。(c) prompt cache 影响经实测坐实:opencode `provider/transform.ts` 的 `applyCaching` 对 `openaiCompatible` 路径**有**注入 `cache_control:{type:ephemeral}`、且 mob-ai 的 `usage` 含 Anthropic 缓存字段位(`cache_creation_input_tokens`/`cache_read_input_tokens`),但发两次完全相同的 18214-token 长前缀(带 ephemeral 标记)后两次缓存读写计数**恒为 0** —— 失效发生在网关后端(litellm 池子不写/不读 ephemeral 缓存),assets-produce 代码层不可控。用户决策(brainstorming 选项 1,授权本修订):主脑 Claude 改经 mob-ai(OpenAI 兼容),**接受 Anthropic ephemeral prompt cache 在此路径失效**(prompt cache 是成本/延迟优化、非功能正确性,失效不影响任何功能;且这是公司合规网关、与 n2m/backend 全平台统一)。落地物:根 `opencode.jsonc` 新增 provider `mob-ai`(`@ai-sdk/openai-compatible`,`env:["MOB_AI_API_KEY"]`,`options.baseURL=https://ai.mob-ai.cn/v1`,`models`: `claude-opus-4-6:free`=默认、`claude-sonnet-4-6:free`、`claude-opus-4-7:free`，三个 `:free` 型号均本 session 实测 HTTP 200 正常回包）+ 顶层 `model:"mob-ai/claude-opus-4-6:free"`（对齐用户 `~/.zshrc` `cc()` 主模型 `ANTHROPIC_MODEL`）(`Provider.defaultModel()` 经 `cfg.model` 解析,`business/asset-service/llm-generator.ts::defaultResolveModel` 主脑与 `agent run` 默认模型均受用);`MOB_AI_API_KEY` 仅写 `.env`(gitignored)+ `.env.example` 空占位,**绝不进 git-tracked 配置**。DeepSeek fallback 链不变。影响范围:LLM 主脑接入端点 + prompt cache 让位;**不改** §2 原子能力 + skill 编排原则、**不改** §11.4 任何对外接口(无 REST/DB/AssetKind/`AssetServiceErrorCode`/OpenAPI 变更)、不动 Phase 14 编排循环逻辑(仅 provider 与默认 model 配置变)。§2/§8「Claude 主脑,开 prompt cache」中"开 prompt cache"经 mob-ai 实测不可达,本条正式记录该偏离;若未来需 prompt cache 收益,另起独立项(评估 litellm cache passthrough 网关配置,或对延迟/成本敏感链路加专用直连 provider)。§8 Provider 章节与 §10 Phase 2 验收第 6 条(prompt cache 命中)加 ⚠ 引本行。 | cdotlock + Claude |

| 1.17 | 2026-05-19 | 用户要求把 asset-generation 的 skill body 接上 Langfuse，使其可热改、不部署即生效，并方便版本管理与统一。核实纠正前提：§15 r1.14 把 skill-body loader 定为只读本地 `knowledge/asset-generation/`，是 Phase-8 草稿 + Phase-14「Langfuse 仍按需上传不在本 phase」的**延后债**；而 **§2 原则 4「Skill body 在 Langfuse」+ §5.2「Langfuse 存 skill body、编辑/版本/A-B 全在 Langfuse Web UI、切换 production/staging 通过 label 不需要改代码」本就是原始设计**——故本条是**落地 1.14 延后的 Langfuse 半截、回归 §2/§5.2**，非推翻原则 4。经 council 四声（Architect/Skeptic/Pragmatist/Critic）+ brainstorming 核实风险：skill body 不是普通 prompt，而是带 `## Atomic tools (allowed)` allowlist 的可执行控制流（allowlist 解析失败 → `SkillInfeasibleError`/`GENERATION_REJECTED`，爆炸半径 = 全体创作者）。用户决策（综合「git 仍正本」A 案 + Langfuse 环境）：(1) loader 改为先取 Langfuse `skill_<name>` 的 **`production` label**，miss/连不上/超时 → 回退本地 `knowledge/asset-generation/<name>.md`，**绝不因 Langfuse 不可达而 hard-fail job**；(2) `knowledge/` 仍是 git 受审查正本兼离线兜底兼首推种子，Langfuse `production` 为热改生产层，二者偏离由 parity-check 显式标记（git 不撒谎）；(3) 新增 `agent skills sync asset-generation [--label staging|production] [--check]` 推送/校验命令（复用 `langfuse.createPrompt` + `promptKeyFor`；asset-generation 为 system-profile 固定 const 集，不进 `skills` DB 表）；(4) **promote 到 `production` 前强制 allowlist 解析校验（≥1 已知工具）作为防灾闸**；(5) 短 TTL（默认 ~60s）进程内缓存，避免每 job 打 Langfuse / 把 job 延迟耦合到 Langfuse。残留项：内容质量回归（golden-asset eval，防「静默变差」）council 标为真缺口，记为本条**开放后续项**（另起独立 §15/计划），本轮不阻塞、不删。详细设计见 `docs/superpowers/specs/2026-05-19-langfuse-skill-loader-design.md`，实施计划见 `2026-05-19-langfuse-skill-loader-plan.md`。影响范围：asset-generation skill-body 加载源（注入缝 `llm-generator.ts` `overrides.loadSkill`/`defaultLoadSkill` 后实现 + 新 sync CLI + parity check + 缓存）；**不改** §11.4 任何对外接口（无新 AssetKind / 无 REST / DB schema / error code 变更）、**不改** §2 原子能力 + skill 编排原则（loader 仍喂同形 skill body，零 loop / 零原子工具改动）、不改 Phase 14 状态机、回归并落地 §2 原则 4 / §5.2。 | cdotlock + Claude |

| 1.18 | 2026-05-19 | 用户要求把 `cdotlock/novels-to-lunascript` 的**上游写作流水线**（从"选小说"到产出 `.ls` 剧本）全部迁进 assets-produce 自维护，n2m 上游退役。Brainstorming 锁定 8 项决策：D1 范围=**仅写作半段**（`novel-evaluator`→…→`episode-writer`/各 reviewer，出 `.ls` 即止；下游 `asset-prompt-generator`/`asset-reviewer`/`music-normalizer`/`sfx-normalizer`/`outfit-anchor-renderer`/`wardrobe-consolidator`/全部 `dramatizer/` **不迁**，仍在 n2m）；D2 创作正文**逐字冻结**进 `knowledge/novel-to-ls/`（零改写零漂移，沿用 B1 D1 对内容的精神）；D3 驱动=**顶层 `novel_to_ls` 编排 skill + agent 驱动**，零流水线代码（否决复用 Phase-14 asset-service 循环 / 否决 per-stage CLI 为主驱动）；D4 注册进**通用 skill 系统**（§5 Skill 表 + `skill <name>` 工具 + system prompt 注入），**不**进 `ASSET_GENERATION_SKILLS`/`intent-to-skill.ts`（本流程非素材生成；附带把与并发 B1 的代码重叠降到≈0）；D5 产物**以 on-disk `lunascripts/<book>/NN-stage/` 布局为权威**（直接按 n2m 布局写盘，`Asset` 表仅可选索引，无导出步），因下游不迁、兼容是硬约束；D6 把 n2m LS 校验器**冻结成 `ls-validate` 原子工具**（`cg-render`/B1 冻结子进程同款）；D7 n2m 上游 10 个 authoring skill 加 `DEPRECATED` 头注释（**不删**，Phase 9/13 先例；push 到 `cdotlock/novels-to-lunascript` **须用户 ack**）；D8 reviewer 用 opencode 既有 `tool/task.ts` 派**独立 fresh-context 子 agent**，PASS/CONDITIONAL/FAIL 评审门语义逐字复刻，无新派发代码。新开 **C 轨**，拆 phase **C0–C4**（C0 设计冻结 / C1 冻结+注册+摄入模型 / C2 编排 skill+reviewer 接线 / C3 `ls-validate`+demo 书端到端+兼容验收 / C4 n2m 退役+文档）。完整设计见 `2026-05-19-upstream-authoring-migration-design.md`。Git：本轨在 worktree `claude/admiring-wilson-5d9f34`（由 `main@659fbdb` 切出）实施，与 main 上并发的 B1 任务隔离，合并前 rebase + §15 追加式手工对账——此为 §11.5 / CLAUDE.md trunk-based 规则的 session 级偏离，经用户明确指令授权。影响范围：新增 1 原子工具（`ls-validate`）+ 11 frozen skill body + 1 编排 skill + 通用注册 + 摄入/workspace 胶水；**不改** §2 原子能力+skill 编排原则（编排为知识体、非硬编码 service）、**不改** §11.4 任何对外接口（无新 AssetKind / 无 REST·DB·error code·OpenAPI 变更）、不动 Phase 2/3、不动 Phase 14 LLM 循环（零 loop 代码改动）、不动 lunaverse-backend。 | cdotlock + Claude |
