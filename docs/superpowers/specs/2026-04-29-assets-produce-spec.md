...
| 1.18 | 2026-05-19 | 用户要求把 `cdotlock/novels-to-moonscript` 的**上游写作流水线**（从"选小说"到产出 `.mss` 剧本）全部迁进 assets-produce 自维护，n2m 上游退役。Brainstorming 锁定 8 项决策：D1 范围=**仅写作半段**（`novel-evaluator`→…→`episode-writer`/各 reviewer，出 `.mss` 即止；下游 `asset-prompt-generator`/`asset-reviewer`/`music-normalizer`/`sfx-normalizer`/`outfit-anchor-renderer`/`wardrobe-consolidator`/全部 `dramatizer/` **不迁**，仍在 n2m）；D2 创作正文**逐字冻结**进 `knowledge/novel-to-mss/`（零改写零漂移，沿用 B1 D1 对内容的精神）；D3 驱动=**顶层 `novel_to_mss` 编排 skill + agent 驱动**，零流水线代码（否决复用 Phase-14 asset-service 循环 / 否决 per-stage CLI 为主驱动）；D4 注册进**通用 skill 系统**（§5 Skill 表 + `skill <name>` 工具 + system prompt 注入），**不**进 `ASSET_GENERATION_SKILLS`/`intent-to-skill.ts`（本流程非素材生成；附带把与并发 B1 的代码重叠降到≈0）；D5 产物**以 on-disk `moonscripts/<book>/NN-stage/` 布局为权威**（直接按 n2m 布局写盘，`Asset` 表仅可选索引，无导出步），因下游不迁、兼容是硬约束；D6 把 n2m MSS 校验器**冻结成 `mss-validate` 原子工具**（`cg-render`/B1 冻结子进程同款）；D7 n2m 上游 10 个 authoring skill 加 `DEPRECATED` 头注释（**不删**，Phase 9/13 先例；push 到 `cdotlock/novels-to-moonscript` **须用户 ack**）；D8 reviewer 用 opencode 既有 `tool/task.ts` 派**独立 fresh-context 子 agent**，PASS/CONDITIONAL/FAIL 评审门语义逐字复刻，无新派发代码。新开 **C 轨**，拆 phase **C0–C4**（C0 设计冻结 / C1 冻结+注册+摄入模型 / C2 编排 skill+reviewer 接线 / C3 `mss-validate`+demo 书端到端+兼容验收 / C4 n2m 退役+文档）。完整设计见 `2026-05-19-upstream-authoring-migration-design.md`。Git：本轨在 worktree `claude/admiring-wilson-5d9f34`（由 `main@659fbdb` 切出）实施，与 main 上并发的 B1 任务隔离，合并前 rebase + §15 追加式手工对账——此为 §11.5 / CLAUDE.md trunk-based 规则的 session 级偏离，经用户明确指令授权。影响范围：新增 1 原子工具（`mss-validate`）+ 11 frozen skill body + 1 编排 skill + 通用注册 + 摄入/workspace 胶水；**不改** §2 原子能力+skill 编排原则（编排为知识体、非硬编码 service）、**不改** §11.4 任何对外接口（无新 AssetKind / 无 REST·DB·error code·OpenAPI 变更）、不动 Phase 2/3、不动 Phase 14 LLM 循环（零 loop 代码改动）、不动 moonshort-backend。 | cdotlock + Claude |
