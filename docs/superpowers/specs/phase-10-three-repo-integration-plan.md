...
|---|---|---|
| 三仓接通范围 | novels-to-lunascript：只调 lookup；lunaverse-backend：4 个操作全用 | 设计 §6 / §7 |
| 共享 library | 不引入；三仓 client 各自实现 | 设计 §2.2 非目标第 2 条 / §11 Phase 10 "不做" 第 4 条 |
| lunaverse-backend 改动面 | 仅 `agent-forge-client.ts` real branch + 新 `assets-produce-http.ts`；不动 `assets-remix-service.ts` 接口 | 设计 §7.1 / §7.2 |
...
| novels-to-lunascript LS schema | `asset_ref` 加可选字段 `id` / `name` / `kind`，向后兼容；旧 LS 仍合法 | 设计 §6.2 |
| novels-to-lunascript lookup 触发 | 默认 off；CLI flag `--resolve-assets` on | 设计 §6.2 / §11 Phase 10 范围 |
...
| 失败重试 | assets-produce 内不自动重试；backend 用既有 BullMQ；novels-to-lunascript 仅 warn 不重试 | 设计 §9.2 |
...
- `ASSETS_API_TOKEN_NOVELS_TO_LUNASCRIPT` → 允许 `novel_*` 前缀 project_id（或具体 allowlist）
- `ASSETS_API_TOKEN_LUNAVERSE_BACKEND` → 允许 `novel_*` 前缀 project_id（小说 → episode → asset 链路；与上同范围）
...
- novels-to-lunascript README 新增一节 "下游 assets-produce 接入"：
  - 写明 `ASSETS_PRODUCE_*` 配置
  - 写明 `ls-verify --resolve-assets` 示例
...
- novels-to-lunascript：`git status` 干净；现有测试全过
- lunaverse-backend：`git status` 干净；现有测试全过；记录 `agent-forge-client.ts` 当前内容；记录 `ASSETS_REMIX_MODE` 在仓内的引用点
- 三仓的 OSS bucket / 凭据是否 alignment（assets-produce 写、其他读）

测试：

- assets-produce: `bun --cwd=agent run test`
- novels-to-lunascript: 项目实际 test 入口（pytest / npm test / etc.）
- lunaverse-backend: 项目实际 test 入口
...
### Step 3 — novels-to-lunascript client 实现
...
### Step 4 — novels-to-lunascript LS schema 调整 + asset_ref 写出器

预期输出：

- LS schema 中 `asset_ref` 加可选 `id` / `name` / `kind`
- schema validator 同步
- asset_ref 写出器在生成过程中填 `kind` 与 `name`
- 旧 LS（不带新字段）仍合法（schema test 验证）

测试：

- schema validation：新 LS + 旧 LS 都通过
- asset_ref 生成器单元测试：给定上下文 → 期望的 `kind` / `name`
- 现有 LS 生成 e2e 测试全过

### Step 5 — novels-to-lunascript `ls-verify --resolve-assets` flag

预期输出：

- CLI 加 flag；on 时调 lookup 填 url；off 时行为不变
- 输入 LS 保留；输出到 `<input>.resolved.json`
- 未命中 query 仅 warn

测试：

- CLI 单元测试：flag on + mock client 返回部分命中 → 输出 LS 部分 url 被填
- flag off → 输出 LS 与输入相同
- 未命中的 asset_ref → stderr warn + 仍写出（不 fail）

### Step 6 — novels-to-lunascript 配置 + README
...
### Step 7 — lunaverse-backend `assets-produce-http.ts` 实现
...
### Step 8 — lunaverse-backend `agent-forge-client.ts` real branch 改造
...
### Step 9 — lunaverse-backend 配置 + README
...
- 在本机起：
  ...
  - lunaverse-backend dev 模式 + `ASSETS_REMIX_MODE=real` + 指向本地 assets-produce
  - novels-to-lunascript 跑一个 fixture 小说 → 生成 LS（含 asset_ref）→ ls-verify --resolve-assets
- 流程：
  ...
  6. 用 novels-to-lunascript ls-verify --resolve-assets 拉回相同 key → url 拿到
...
- novels-to-lunascript：commit + push（remote 应在用户 namespace；无需额外 ack；如不在，同样需问）
- lunaverse-backend：commit 已就位；push **必须** backend 维护方明确同意后再推
...
| novels-to-lunascript 与 assets-produce key 命名不一致导致 lookup 大量 miss | 中 | 中 | Step 4 起 key 命名约定文档（`docs/ops/three-repo-token-flow.md` 同处写）；两侧测试用同套 fixture key |
...
| LS 旧文件被新 schema 拒绝 | 中 | 中 | 新字段一律 optional；新增 schema 测试用旧 fixture 校验 |
...
- [ ] novels-to-lunascript `assets_produce_client.py` 单元覆盖 happy / 404 / 401 / 403 / 网络错
...
- [ ] 用户本机一条 e2e 走通：LS 生成 → backend remix → assets-produce real → 拿到 url → 小程序看到图
...
- [ ] novels-to-lunascript 所有 atomic commit 已 push（按用户 namespace 规则）
- [ ] lunaverse-backend 所有 atomic commit 已就位；push 须 backend 维护方明确 ack 后执行
