...
| `AugustZAD/Dramatizer-LS` (novels-to-lunascript) | main, 3 commits ahead at `9d151f7..b2eb443`, pushed | 35/35 new (19 client + 16 resolve CLI) |
| `cdotlock/lunaverse-backend` | local-only, 3 commits at `c4c85eb..7b1a00e`, **NOT pushed** | 73/73 `__tests__/upstream/` (45 new + 28 pre-existing) |

Plan deviation: §1.2.2 (LS schema 加 `asset_ref` 节点字段) was dropped — schema is owned by the out-of-scope Go binary `ls` (cdotlock/lunascripts). §1.2.4 (`ls-verify --resolve-assets`) was adapted onto n2m's actual asset-injection mechanism, `mapping.json`. Net effect: same integration goal, achieved via the real plumbing. See [§ 5 Plan deviations](#5-plan-deviations).
...
### 1.2 novels-to-lunascript (pushed to origin/main)
...
### 1.3 lunaverse-backend (local only, NOT pushed — user policy)
...
- [x] **novels-to-lunascript AssetsProduceClient unit coverage** — 19 tests in `dramatizer/tests/test_assets_produce_client.py`:
...
- [x] **lunaverse-backend assets-produce-http.ts unit coverage** — covered in `__tests__/upstream/assets-produce-http.test.ts` (passes vitest run; combined with `agent-forge-client.test.ts` → 45 tests for Phase 10 + 28 for the surrounding upstream files = 73 green).
...
- [⚠️ partial] **One local e2e: LS → backend remix → assets-produce real → URL → app**. Two halves:
...
- [x] **novels-to-lunascript commits pushed** — see § 1.2.
- [x] **lunaverse-backend commits local only, push gated on user ack** — see § 1.3.
...
| ID | What | Where | Owner |
|---|---|---|---|
| F1 | Push the 3 backend commits + the Phase 9 DEPRECATED notice file to `cdotlock/lunaverse-backend` | backend repo | user (coordinate with backend maintainer per global CLAUDE.md "cross-namespace push needs ack") |
...
| F3 | Optional: extend the resolve-assets CLI with an `--in-ls` mode if the Go `lsc` binary later exposes an explicit `asset_ref` AST | n2m | future |
| F4 | Phase 10 pre-existing test failure: `dramatizer/tests/test_compile_ls.py::test_compile_all_rejects_md_without_episode_header` fails on `main` independently of Phase 10 work (verified by `git stash` of new files) | n2m | maintenance, separate ticket |
...
### 5.1 §1.2.2 LS schema — dropped (out of scope)

The plan proposed adding optional `id` / `name` / `kind` fields to an `asset_ref` node in LS .md syntax. In reality the LS schema is owned by the **Go** binary `lsc` at `cdotlock/lunascripts`, which n2m shells out to via `lsc compile --assets mapping.json`. Touching that schema crosses the same "cdotlock push needs ack" line as the backend, with no immediate caller. Dropped for Phase 10; if asset-resolution semantics ever need first-class LS support, this is a follow-up ticket against lunascripts.

### 5.2 §1.2.4 `ls-verify --resolve-assets` — adapted to `mapping.json`

The plan assumed an existing `ls-verify` CLI in n2m to extend. No such CLI exists today. n2m's real asset-injection point is the flat `{key: url}` dict in `dramatizer/build/<slug>/mapping.json` consumed by the Go binary. The new `dramatizer/pipeline/resolve_assets.py` module covers the same conceptual flow (`--resolve-assets` on, off by default) but operates on `mapping.json` directly. Same integration semantics, real plumbing.
...
| Old LS files rejected by new schema | N/A — schema change dropped (§5.1). |
...
Phase 10 closes the **mechanical** integration between assets-produce, novels-to-lunascript, and the lunaverse-backend client layer. The remaining production gates (backend repo push, live cross-stack e2e, one-week observation) are tracked as F1 / F2 follow-ups and depend on user-side coordination, not on this Phase 10 work.
