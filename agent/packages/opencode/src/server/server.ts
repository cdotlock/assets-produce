...
import { assetServiceHttpApp } from "@/business/asset-service/wire"
...
function createHono(opts: { cors?: string[] }) {
  ...
  return {
    app: app
      ...
      .route("/", workspaceApp)
      .route("/", InstanceRoutes(runtime.upgradeWebSocket))
      // Asset Service public surface (Phase 8). Mounted at root so its own
      // /api/v1/assets/* prefix maps directly. Auth is handled inside the
      // sub-app via its Bearer middleware — independent of WebAuth above.
      .route("/api/v1/assets", assetServiceHttpApp())
      .route("/", UIRoutes()),
    runtime,
  }
}
