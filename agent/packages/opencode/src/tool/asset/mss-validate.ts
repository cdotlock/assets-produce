...
import DESCRIPTION from "./mss-validate.txt"

const TOOL_ID = "mss-validate"
...
export const Parameters = Schema.Struct({
  script_path: AbsolutePath.annotate({
    description:
      "Absolute path to the .mss (or .md) file to validate. Must be an absolute path; relative paths are rejected.",
  }),
  mock: Schema.optional(Schema.Boolean).annotate({
    description:
      "Run the bridge in mock mode (hermetic, no Go toolchain required). Use in tests. Returns a canned PASS unless script_path ends with __MSS_MOCK_FAIL__.",
  }),
...
// Validated shape of mss_validate.py's stdout on success (both PASS and FAIL
...
//   PASS: {"verdict":"PASS","errors":[],"meta":{"atomic_tool":"mss-validate","mock":<bool>}}
//   FAIL: {"verdict":"FAIL","errors":[...],"raw":"...","meta":{"atomic_tool":"mss-validate","mock":<bool>}}
...
export const MssValidateResult = Schema.Struct({
...
export interface MakeMssValidateToolOpts {
  runner?: PythonRunner
  /** Absolute path to `tools/mss-validate/mss_validate.py`. Defaults to repo-relative. */
  scriptPath?: string
}

const REPO_ROOT = path.resolve(path.dirname(new URL(import.meta.url).pathname), "../../../../../..")
const DEFAULT_SCRIPT = path.join(REPO_ROOT, "tools", "mss-validate", "mss_validate.py")

export function makeMssValidateTool(opts: MakeMssValidateToolOpts = {}) {
      ...
      return {
        ...
        execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context) =>
          ...
          Effect.gen(function* () {
            ...
            // Parse mss_validate.py's stdout JSON. The bridge promises strict
            ...
            const decoded = yield* Schema.decodeUnknownEffect(MssValidateResult)(parsed).pipe(
              Effect.mapError(
                (err) =>
                  new Error(
...
export const MssValidateTool = makeMssValidateTool()
