...
// "knowledge/novel-to-ls" entry resolved against the project directory.
// (Identical resolution to novel-to-ls-discovery.test.ts.)
const REPO = path.resolve(import.meta.dir, "../../../../..")

const SKILL = "novel_to_ls"
...
// (knowledge/novel-to-ls/novel_to_ls/SKILL.md, commits 4e9d33e + c1714c5),
...
describe("novel_to_ls orchestration skill discovery", () => {
  ...
  // "knowledge/novel-to-ls" against Instance.directory, and scans the
  // on-disk SKILL.md files. No stubbing of discovery or the filesystem.
  it.live("opencode discovery sees the novel_to_ls orchestration skill, filesystem-served", () =>
    Effect.gen(function* () {
      ...
      expect(info.location).toContain(path.join("knowledge", "novel-to-ls", "novel_to_ls"))
...
const BODY_PATH = path.join(REPO, "knowledge", "novel-to-ls", "novel_to_ls", "SKILL.md")
...
// registered `ls-validate` tool as the post-stage-5 `.ls` quality gate, which
// requires its own `## .ls quality gate` section. Adding that heading to the
...
// `## .ls quality gate` content guard (its own describe block further down)
...
describe("novel_to_ls body encodes the full stage + gate contract", () => {
  ...
  test("all required `## ` sections are present (exact headings)", () => {
    ...
    // by one in lock-step with C3 Task 5's additive `## .ls quality gate`.)
...
// ── .ls QUALITY GATE WIRING GUARD (C3 Task 5) ─────────────────────────────
//
// C3 froze the upstream LS validator as the registered atomic tool
// `ls-validate` (tool id "ls-validate"; returns {verdict:"PASS"|"FAIL",
...
// orchestration body as the post-stage-5 per-episode `.ls` quality gate,
// ADDITIVELY (its own `## .ls quality gate` section) so no prior C2 invariant
// breaks. This guard pins the wired contract the body must encode:
//
//   - the section names the registered `ls-validate` tool by id;
//   - it is positioned as the gate AFTER stage-5 `episode-writer` writes
//     `05-episode-writer/scripts/<ep>.ls`;
...
//     (「每集 FINAL 之前必须 `lsc compile` exit 0」) as the upstream source
//     authority this gate enforces;
//   - it stays KNOWLEDGE: it directs use of the already-registered
//     `ls-validate` tool and introduces no code / engine / service.
//
// Non-vacuity: every assertion is scoped to the `## .ls quality gate` section
...
// edit was appending ".ls quality gate" to REQUIRED_SECTIONS (documented at
...
describe("novel_to_ls body wires ls-validate as the post-episode .ls quality gate (C3 Task 5)", () => {
  const body = readFileSync(BODY_PATH, "utf8")
  const sections = parseSections(body)

  test("a dedicated `## .ls quality gate` section exists and is non-empty", () => {
    expect(sections.has(".ls quality gate"), "body must add a `## .ls quality gate` section").toBe(true)
    expect(
      (sections.get(".ls quality gate") ?? "").trim().length,
      "`## .ls quality gate` section must not be empty",
    ).toBeGreaterThan(0)
  })

  test("the section names the registered `ls-validate` tool as the post-stage-5 `.ls` gate", () => {
    const gate = sections.get(".ls quality gate")!
    // Names the registered atomic tool by its exact id.
    expect(gate, "must reference the registered `ls-validate` tool by id").toContain("`ls-validate`")
    // Anchored to stage-5 episode-writer's produced `.ls` under the C1 dir.
    ...
    expect(gate.toLowerCase(), "must talk about the produced episode `.ls`").toContain(".ls")
  })
  ...
  test("the section cites the C1-frozen episode-writer hard gate as the upstream source authority", () => {
    ...
    // The frozen rule it enforces: 「每集 FINAL 之前必须 `lsc compile` exit 0」.
    expect(
      gate.includes("ls compile") && gate.includes("exit 0"),
      "must quote the frozen episode-writer hard gate (`lsc compile` exit 0)",
    ).toBe(true)
  })

  test("the section stays knowledge: declares it adds no code / engine / service", () => {
    const gate = sections.get(".ls quality gate")!.toLowerCase()
 ...
 *  knowledge/novel-to-ls/bible-reviewer/SKILL.md `## 输出格式` / `## 总结论`:
...
describe("novel_to_ls gate halts on injected FAIL verdict", () => {
  const body = readFileSync(BODY_PATH, "utf8")
