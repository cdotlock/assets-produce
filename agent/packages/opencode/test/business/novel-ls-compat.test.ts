 ...
 *     in CI — C1 precedent).
 *
 * (b) Golden input-contract: for each committed ls-golden fixture, invoking
 *     ls-validate in hermetic mock mode (--mock, no Go) must yield a
 *     well-formed LsValidateResult. Real-validator verdict is deferred to
...
import { makeLsValidateTool, LsValidateResult } from "@/tool/asset/ls-validate"
...
//
// Recorded from the n2m authoring subset of `lunascripts/no-rules-in-bad-ideas/`
...
// is narrowed to its `scripts/` .ls output dir. See workspace.ts:4-6 for the rationale.
...
const GOLDEN_DIR = path.resolve(
  import.meta.dirname,
  "../fixture/novel-to-ls/ls-golden",
)
const GOLDEN_FILES = [
  "ep_10_weston_final.ls",
  "ep_10_diego_final.ls",
  "ep_11_luca_final.ls",
] as const

// ─── Effect runtime (mirrors ls-validate.test.ts) ────────────────────────────
...
describe("novel workspace NN-stage parity", () => {
  ...
  test("ensureNovelWorkspace materializes all stage dirs + skills/arc-reviewer", () => {
    ...
    try {
      ...
      const base = path.join(root, "lunascripts", "demo-book")
...
describe("ls-golden input-contract (hermetic mock mode)", () => {
  for (const filename of GOLDEN_FILES) {
    ...
    test(`${filename} — ls-validate mock mode yields well-formed LsValidateResult`, async () => {
      ...
      // We assert only: tool returns a LsValidateResult that decodes against the schema.
      const info = await runtime.runPromise(makeLsValidateTool())
      ...
      // The output must be a well-formed LsValidateResult JSON string.
