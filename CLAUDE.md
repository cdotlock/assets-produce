...
- **asset-generation skill body 加载（spec §15 r1.17）**：运行时 loader 先读 Langfuse `production` label，miss/error/timeout/无凭据 → 回退 `knowledge/asset-generation/<name>.md`（git 正本 + 兜底 + 首推种子）。**Langfuse 不可达绝不 hard-fail job**。进程内 TTL 缓存 `ASSETS_SKILL_LANGFUSE_TTL_MS`（默认 60000ms）。
