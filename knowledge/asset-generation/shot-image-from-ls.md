...
lunascripts (LS) document.

This is the most common asset kind during the novels-to-lunascript →
lunaverse-backend → assets-produce pipeline. LS authors mark
`asset_ref` nodes with a `name` and a `kind` of either `shot_image` or
`shot_video`; the lunaverse-backend resolver eventually calls
...
- `intent.spec_md` — the asset_ref's `note` / `prompt` field from the
  LS document. Look for: action verb, blocking, framing, dialogue
  intent.
- `intent.refs[tag=="character"]` — character立绘 references resolved
  from upstream Asset rows by the LS asset-ref resolver.
