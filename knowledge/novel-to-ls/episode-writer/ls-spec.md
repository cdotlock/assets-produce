...
- `pos`：**只能是 `left` | `center` | `right`**（白名单，三个值，没有别的）
...
> **历史包袱**：早期 spec 列过 `left_far` / `right_far` 给"第三角色 / 远处"用，但 ls parser 从来不接受这两个值，写出来 `lsc compile` 直接 reject。本舞台只有 3 个槽（见 §4.2 单角色露出规则），第三角色用 left/center/right 任一即可。**不要写 `left_far` / `right_far`、也不要写任何语义化位置词（`door` / `window` / `behind` / `near_door` ...）**——它们都不是合法 token，parser 会拒。
...
##### 单角色露出可见性规则（硬性，**写脚本必须清楚**）

引擎遵循 **"单槽露出 + 说话者优先"** 规则（实现见 `lunaverse-backend/app/(player)/play/[sessionId]/page.tsx` 的 `activeSlot` 逻辑）：

舞台同时维护 left / center / right 三个槽，但**任何一帧只有 1 个槽 opacity = 1**，其它两个 opacity = 0。哪个槽露出由当前显示行决定：

| 当前行类型 | 露出哪个角色 |
|---|---|
| `CHARNAME: 台词` (dialogue) | `CHARNAME` 露出 |
| `YOU: 内心独白` | **MC（本项目 = `selena`）** 露出 |
| `NARRATOR: 旁白` | **最近被 `@show` / `@look` 触碰的那个槽**露出 |
| `@pause for N` | 同 NARRATOR — 最近触碰的槽 |

**直接推论：`@<char> look X` 必须在 X 真的会被显示前才有意义。**

X 会被显示的条件（任一）：
1. 紧接着 `<char>` 自己说一句台词（dialogue 行）
2. `<char>` 是 MC（即 selena），且紧接着是 `YOU:` 行
3. 紧接着是 `NARRATOR:` / `@pause`，并且 `<char>` 是最近被触碰的槽（即这个 look 之后到第一个 narrator/pause 之间，没有其它 `@<other> show/look`）

**Dead Look — 协议禁止的反模式**：

如果一个 `@<char> look X` 满足下列任一，X 永远**不会显示**，**禁止写**：

```
@<char> look X
@<char> look Y                ← X 未显示就被覆盖
```

```
@<char> look X
SOMEONE_ELSE: ...             ← 别人在说话
@<char> hide                  ← <char> 离场，X 没机会显示
```

```
@<char> look X
@<other> look Z               ← <other> 抢走"最近触碰"
NARRATOR: ...                 ← 此刻露出 <other>，不是 <char>
@<char> look Y                ← X 整段从未显示
```

正确替代：要么删掉这个 look（它本来就没意义），要么把它改写成 `<char>` 真正会露出的位置（比如挪到他自己说话之前）。

**验证工具**：`python3 skills/episode-writer/look_audit.py <ep>.md` 会扫出所有 dead look，标 H 高亮。FINAL 前必须确认 0 个 dead look。
...
`pos` 同 `show` 指令——**只能是 `left` | `center` | `right`**。`@weston move to door` / `@x move to behind` 这种语义化目标不是合法 token，parser 会报 `[INVALID_POSITION]`。要表达"走到门口"这种叙事意图，用 NARRATOR 旁白写动作，再 `@<char> hide` 让他退出舞台；下一帧需要的话再 `show`。
...
CG 在下游由 agent-forge 渲染成短视频。LS 是流程控制 + 内容，所以脚本必须携带视频管线需要的叙事素材——镜头走向和故事拍子。
...
互动 VN 是当事人视角，玩家**就是** MC。NARRATOR 引用 MC 时必须用第二人称大写代词，不许用第三人称指 MC。

| 场景 | ❌ 错 | ✅ 对 |
|---|---|---|
| EN 主格 | `She walks into the room.` | `YOU walk into the room.` |
| EN 物主 | `Her phone buzzes.` | `YOUR phone buzzes.` |
| EN 宾格 | `Easton hands her the key.` | `Easton hands YOU the key.` |
| EN 直呼 MC 名 | `Selena's heart races.` | `YOUR heart races.` |
| ZH 主格 | `她走进房间。` | `YOU 走进房间。` |
| ZH 物主 | `她的手机震动。` | `YOUR 手机震动。` |

ZH 版**保留 `YOU` / `YOUR` / `YOURS` 英文 token**（不译成"你/你的"），与 CJK 之间用空格隔开。其他女性角色（Elena/Ximena/Sofia 等）用回正常代词，不受此规则影响。

`YOU:` 内心独白行内部就是第一人称（"my heart" / "我的心"），是 MC 自述，不与 NARRATOR 规则冲突。

> 该约束由 `skills/episode-writer/check_narrator_pov.py` 在每集 FINAL 前强制扫描；`--fix` 模式可自动改写绝大多数 EN/ZH 违规。
...
- `safe`：跳过检定，块内直接是叙事内容
- `<text>`：选项按钮 label，**必须是短语 / 意图描述**，不是台词全文
...
|---|---|---|
| 英文 | **80 字符** | 含空格、标点 |
| 中文 | **60 列宽** | CJK = 2 列宽，ASCII = 1 列 |

UI 选项按钮承载"玩家此刻要做什么"的**意图概要**，不是 MC 完整台词。台词写在 option block 内部（`SELENA:` / `YOU:`），label 只是按钮上的一行字。由 `check_narrator_pov.py` 同时扫描，超长直接 fail。
...
- 解释器将两者结合：`lsc compile script.md --assets mapping.json -o output.json`
...
```bash
lsc compile 01.md --assets mapping.json -o ep01.json        # 单集编译
lsc compile main/ --assets mapping.json -o novel.json        # 批量编译整个目录
lsc validate 01.md --assets mapping.json                     # 验证（不输出 JSON）
```
