...
- character-architect 输出完整,所有 bible 在 `lunascripts/{book-slug}/02-character-architect/` 下
- bible-reviewer 对 bible 出 **PASS** 判决(不接受 CONDITIONAL 或 FAIL)
- 用户已告知目标**总集数**(15-30 之间为常见)
- 已通读 [`../episode-writer/ls-spec.md`](../episode-writer/ls-spec.md) 了解 @signal / @affection / @butterfly / @gate 怎么工作
...
LS 规范要求每集要么有 `@gate` 继续路由,要么有 `@ending` 终结。写 plan 时每个"终点集"必须明确它是哪种终点:
...
所有文件落在 `lunascripts/{book-slug}/03-entity-planner/`:
