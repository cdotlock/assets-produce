...
# novel_to_ls — Upstream Authoring Orchestration
...
contract. `<base>` = `<workspace-root>/lunascripts/<book-slug>/`. The slug must
...
Post-LS stages (02.5 / 05.5 / 06 / ls-build) are intentionally **out of
scope** for this orchestration body (C-track boundary). Stop at `.ls scripts`.
...
do **not** prove the produced `.ls` actually parses. The C1-frozen
...
`lsc compile` exit 0"): 「光读 spec / 跑 review 都不够 …… 只有 ls 编译器的判决是真理。
...
The enforcement uses the **already-registered** atomic tool `ls-validate`
(opencode tool id `ls-validate` — the C3-frozen wrapper around the upstream LS
...
`05-episode-writer/scripts/<ep>.ls` (the C1 `05-episode-writer/scripts` dir —
...
`ls-validate` tool on that produced episode `.ls`. Every produced episode
`.ls` is gated this way — no sampling.

**Verdict → action (same shape as a reviewer CONDITIONAL — see `## Gate
contract`):**

- `verdict:"PASS"` → the `.ls` parses; that episode may proceed toward FINAL
...
- A **non-PASS** verdict — `verdict:"FAIL"`, or a tool `metadata.error` — is
  ...
  agent **re-runs** `ls-validate` on the corrected `.ls`; this is a **loop**
...
`arc-reviewer`) AND obtained an `ls-validate` `verdict:"PASS"`. Never present a
...
agent to call the **already-registered** `ls-validate` tool and to apply its
