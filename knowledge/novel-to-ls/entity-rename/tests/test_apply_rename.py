...
def test_speaker_label_pair():
    """Uppercase character ID in prose should be rewritten (prose emphasis + LS speaker labels)."""
    rename_map = {
        "characters": {"alice": {"new_id": "nova", "new_full_name": "Nova",
                                  "alias_mapping": {}}},
        "locations": {}, "free_tokens": {},
...
def test_ls_token_pair():
    ...
    normalizer_chars = {"alice": {"full_name": "Alice", "aliases": []}}
    pairs = build_replacement_pairs(rename_map, normalizer_chars, {})
    assert any(p.kind == "ls_token" and p.replacement == "@nova" for p in pairs)
...
def test_apply_to_text_ls():
    from apply_rename import apply_to_text

    pairs = [ReplacementPair(re.compile(r"@alice\b"), "@nova", "ls_token")]
