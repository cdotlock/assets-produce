...
class IntegrityFail(Exception):
    """LS tokens or speaker labels reference unknown IDs."""
...
def _scan_ls_problems(root: Path) -> List[str]:
...
def _find_latest_backup(project_dir: Path) -> Optional[Path]:
    ...
    Timestamps are ``YYYYMMDD-HHMLS`` so lexicographic sort is chronological.
...
def check_ls_integrity(
    ...
    full state.
    """
    current = _scan_ls_problems(project_dir)
    if backup_dir is None or not backup_dir.exists():
        return current
    baseline = set(_scan_ls_problems(backup_dir))
...
def main(argv: list[str]) -> int:
    p = argparse.ArgumentParser(
        description="Phase R hard checks: residual (hard), LS integrity + "
    ...
    project_dir = Path(args.project_root) / "lunascripts" / args.book_slug
    ...
    # Advisory checks: LS integrity and dead aliases are reported as warnings.
    # LS integrity diffs against the latest backup so pre-existing data gaps
    # (a bug in normalizer, not rename) are not surfaced as rename issues.
    backup_dir = _find_latest_backup(project_dir)
    for w in check_ls_integrity(project_dir, backup_dir=backup_dir):
        print(f"[WARN] LS: {w}", file=sys.stderr)
