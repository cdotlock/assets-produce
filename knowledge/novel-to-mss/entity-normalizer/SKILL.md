...
若 `moonscripts/<book-slug>/04-entity-normalizer/.rename_applied` 存在,说明本目录已被 `entity-rename` 覆写为新名。
...
1. **Bible 包**:`moonscripts/{book-slug}/02-character-architect/`(MC bible + LI bibles + supporting-cast)
2. **Plan 包**:`moonscripts/{book-slug}/03-entity-planner/`(00-structure + 01-common + route files)
...
输出到 `moonscripts/{book-slug}/04-entity-normalizer/`:
...
说明:
- `"malia"` 这个 key 就是 MSS 里的 entity ID,episode-writer 写 `@malia show worried at left` 用的就是它
...
- **MSS 解释器** 用 mapping.json 把 entity ID 翻译成 OSS URL
...
- **不要用中文做 ID**:`entity ID` 必须是英文小写下划线,因为 MSS 语法只认英文 token
