...
# moonshort-backend/app/(player)/play/[sessionId]/page.tsx.
MC_CHAR = "selena"
...
def load_asset_index(project_root: pathlib.Path) -> dict[str, str]:
    ...
    candidates = [
        project_root
        / "moonscripts/no-rules-in-bad-ideas/06-asset-prompt-generator"
        / "_workspace/mapping_per_ep",
        project_root
        / "moonscripts/no-rules-in-bad-ideas/mss-build/episodes-zh",
    ]
    for d in candidates:
        if not d.exists():
            continue
