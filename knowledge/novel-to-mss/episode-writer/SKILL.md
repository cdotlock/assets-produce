---
name: episode-writer
description: 基于character-architect的角色体系,生成 MSS(MoonShort Script)格式的完整互动游戏剧本,含选择设计、D20 检定、@butterfly 蝴蝶效应、@gate 路由和钩子工程,并自评迭代至9分以上。当用户需要写互动游戏剧本时触发。
allowed-tools: Read, Write, Edit, WebSearch
---

# Episode Writer — 互动游戏剧本生成 + 自评迭代

基于 `character-architect` 输出的角色体系,**用 MSS (MoonShort Script) 格式写出完整互动游戏剧本**,然后自评、迭代,直到综合分 ≥ 9.0。
...
- 路线对比矩阵已确定
- 配角筛选已完成
- 熟读完整格式规范 [`mss-spec.md`](mss-spec.md)

**如果没有这些输入,先要求用户运行 `/character-architect` → `/bible-reviewer` 闭环,PASS 后再回来。**

## 输出格式：MSS (MoonShort Script)

**本 skill 输出的剧本必须使用 MSS 格式。**
...
python3 skills/episode-writer/check_narrator_pov.py \
  moonscripts/<book-slug>/05-episode-writer/scripts --summary-only

# 自动修 NARRATOR 人称(EN+ZH);@option 长度只报告不自动改(创意重写)
python3 skills/episode-writer/check_narrator_pov.py \
  moonscripts/<book-slug>/05-episode-writer/scripts --fix
```
...
MSS 里 `@<char> show <look> at <pos>` 是"入场"指令。如果一个角色已经在场景里没 hide,你再 show 一次就是语法歧义——validator 可能报错,引擎行为不确定。
...
**为什么这是个坑**:引擎是**单角色露出 + 说话者优先**(详见 mss-spec.md §4.2 "单角色露出可见性规则")。屏幕同一时刻只有 1 张脸,谁说话谁露出。当前不说话的角色,你给他改 look **完全不会显示**——但 06 资产管线还是会给这个 look 生成 prompt、可能渲一张图。
...
1. 打开 `moonscripts/<slug>/04-entity-normalizer/locations.json`
...
### 上游 parser 硬禁止(不改会 mss validate 报错)
...
### 交付硬门槛:每集 FINAL 之前必须 `mss compile` exit 0

光读 spec / 跑 review 都不够 —— spec 偶尔跟 parser 不同步(早期的 `right_far` 就是这种),review agent 也抓不全语法层的硬错。**只有 mss 编译器的判决是真理**。

```bash
PATH=/Users/august/go/bin:$PATH mss compile <ep>_final.md > /dev/null
```
...
> 提示:`mss fix <ep>.md --check`(干跑模式)会给出比 `mss compile` 更精确的"在哪一行"诊断,且把可自动修复的项归类。出错时先用它定位,再用 `mss compile` 复查终态。
...
- **不要虚高打分**
- **不要使用 Episode 格式**：本 skill 只输出 MSS
