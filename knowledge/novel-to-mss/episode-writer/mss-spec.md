...
> **历史包袱**：早期 spec 列过 `left_far` / `right_far` 给"第三角色 / 远处"用，但 mss parser 从来不接受这两个值，写出来 `mss compile` 直接 reject。本舞台只有 3 个槽（见 §4.2 单角色露出规则），第三角色用 left/center/right 任一即可。**不要写 `left_far` / `right_far`、也不要写任何语义化位置词（`door` / `window` / `behind` / `near_door` ...）**——它们都不是合法 token，parser 会拒。
...
引擎遵循 **"单槽露出 + 说话者优先"** 规则（实现见 `moonshort-backend/app/(player)/play/[sessionId]/page.tsx` 的 `activeSlot` 逻辑）：
