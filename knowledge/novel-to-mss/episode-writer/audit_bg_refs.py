...
Usage:
    python3 skills/episode-writer/audit_bg_refs.py \\
      ...
      --episode moonscripts/<slug>/05-episode-writer/scripts/ep_1_final.md
...
def _load_zenmux_env() -> str:
    """Load LLM API key from moonshort-backend/.env or local .env.
    ...
    candidates = [
        Path(os.environ.get("HOME", "/")) / "MobAI/moonshort-backend/.env",
        Path(".env"),
    ]
    found: dict[str, str] = {}
    ...
    raise RuntimeError(
        ...
        "ZENMUX_API_KEY in env or moonshort-backend/.env"
    )
...
def main() -> int:
    ...
    ap.add_argument("--root", type=Path, default=Path("moonscripts"))
