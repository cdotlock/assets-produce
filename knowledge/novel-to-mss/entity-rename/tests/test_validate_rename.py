...
def test_detects_unknown_mss_token(tmp_path, mini_project, rename_map_path):
    """check_mss_integrity returns a warning list (no longer raises)."""
    from validate_rename import check_mss_integrity
    ...
    problems = check_mss_integrity(work)
    assert any("ghost" in p for p in problems), f"expected @ghost, got {problems}"


def test_mss_integrity_ok(tmp_path, mini_project, rename_map_path):
    ...
    assert check_mss_integrity(work) == []


def test_mss_integrity_ignores_preexisting_when_backup_given(
    tmp_path, mini_project, rename_map_path
):
    """A pre-existing MSS bug (present before apply) is NOT a rename
    ...
    problems = check_mss_integrity(work, backup_dir=backup)
    assert not any("jared" in p for p in problems), (
        f"pre-existing @jared must not be flagged; got {problems}"
    )


def test_mss_integrity_flags_rename_introduced_problem(
    tmp_path, mini_project, rename_map_path
):
...
def test_mss_integrity_without_backup_reports_all(
    tmp_path, mini_project, rename_map_path
):
    ...
    problems = check_mss_integrity(work)  # no backup_dir
