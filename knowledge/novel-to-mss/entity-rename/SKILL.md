---
name: entity-rename
description: 对整本 moonscripts 小说的人物/地点/专有名词改名。三相流程:提候选 → 人工批准 → 机械套用 → sub-agent 审查。前置:04-entity-normalizer 完成。当用户要求版权脱敏、隐喻保持、全本改名,或进入 04.5 阶段时触发。
...
对 `moonscripts/<book-slug>/` 下所有名字类 token 做系统性替换,保证 6 层 pipeline 产出一致。

六类 token:
1. **结构化 JSON** — `04-entity-normalizer/*.json` key/value/bible_file 改写
2. **MSS token** — `@<id>`、`@bg set <id>`、`<ID>:` 大写标签
...
- `moonscripts/<book-slug>/04-entity-normalizer/{characters,locations,alias_map}.json`(shopping list)
- `moonscripts/<book-slug>/02-character-architect/*.md`(bibles — 散文扫描源)
- `moonscripts/<book-slug>/03-entity-planner/*.md`(plans — 散文扫描源)
- `moonscripts/<book-slug>/05-episode-writer/scripts/*.md`(所有剧集 + arc review)
- `moonscripts/<book-slug>/06-asset-prompt-generator/tasks_output.json`

## 输出

`moonscripts/<book-slug>/04.5-entity-rename/`:
- `rename_candidates.md` — Phase A 人读候选
- `rename_map.json` — Phase B 定稿权威映射(永久台账)
- `apply_report.md` — Phase R 审计报告
- `.backups/<YYYYMMDD-HHMMSS>/` — 完整项目备份(可回滚)

`moonscripts/<book-slug>/04-entity-normalizer/`:
...
python3 skills/entity-rename/scripts/scan_tokens.py \
  --book-slug <slug> \
  --project-root . \
  --exclude-known-from moonscripts/<slug>/04-entity-normalizer/characters.json
```
...
落到 `moonscripts/<slug>/04.5-entity-rename/rename_candidates.md`。
...
python3 skills/entity-rename/scripts/validate_map_schema.py \
  --book-slug <slug> \
  --map moonscripts/<slug>/04.5-entity-rename/rename_map.json \
  --project-root .
```
...
**软检查(打印 `[WARN]`,不阻塞 exit 0):**
- MSS 完整性:`@<id>` / `@bg set <id>` 必须解析到已知实体。与 backup 做 diff,
...
2. 语义检查:并发 dispatch 4 个 `rename-reviewer` sub-agent,scope 分别为:
   ...
   - `scope=scripts`:检查 05 MSS token + speaker 对齐 + 对白散文
...
cp -rf moonscripts/<slug>/04.5-entity-rename/.backups/$TS/* moonscripts/<slug>/
rm moonscripts/<slug>/04-entity-normalizer/.rename_applied
```
...
(MSS token)和 `MALIA`(speaker label)被 rename,裸 `malia` 漏过。**规则**:
...
### 10. `check_mss_integrity` 要 diff 备份

Pre-existing MSS 数据缺口(`@jared` 在 rename 前就没在 characters.json 登记)
不是 rename 的锅。check_mss_integrity 会从最近一次 `.backups/<ts>/` 里读
