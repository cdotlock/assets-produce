...
description: 对 episode-writer 产出的 MSS 剧本做独立审查。强制通过 Task 工具 spawn 独立 sub-agent 执行,用新鲜视角检查剧本是否兑现了 bible + plan + Wattpad 写作原则 + MSS 语法规则。输出 episode-review-report.md 并给出 0-10 打分。前置条件:episode-writer 已产出一版剧本。当 episode-writer 写完一版剧本需要独立审查时触发。
...
1. **剧本文件**:要审查的 MSS 文件(通常 `scripts/*-v1.md`)
2. **Bible 包**:`moonscripts/{book-slug}/02-character-architect/`(人设核对)
3. **Plan 包**:`moonscripts/{book-slug}/03-entity-planner/`(场景 beat 核对 + tentpole 清单)
4. **Entity 归一化**:`moonscripts/{book-slug}/04-entity-normalizer/`(角色 ID / 场景 ID 核对)
5. **episode-writer SKILL**(读里面的"自查清单"和"互动游戏≠小说"当扣分依据)
6. **MSS 规范**:`skills/episode-writer/mss-spec.md`(语法合规)
...
2. **先跑硬性 validator(必须 0 violations 才进入 sub-agent 审查)**:
   ...
   python3 skills/episode-writer/check_narrator_pov.py \
     moonscripts/<book-slug>/05-episode-writer/scripts --fail-on-violation
   ```
...
- **视觉冗余**:有没有"他皱起眉头"但 MSS 里已经 `@<char> look <angry>` 了?这种双重描述要砍
...
- `@if (...)` 条件语法合法(choice 用 `A.fail` 不是 `A fail`,其他类型按 mss-spec 5 种)
